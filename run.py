...
def run(train, val, models, lr, t_total, epochs, has_persona, gradient_accumulation_steps, device, fp16, 
    ...
    for epoch in range(epochs):
        ...
        print("\nEpoch {0}: train loss: {1:.4f}, valid loss: {2:.4f}, train_acc: {3:.4f}, valid acc: {4:.4f}, valid recall: {5}, valid_MRR: {6:.4f}"
...
og_data = PersonaChatTorchDataset('/home/stc/amatveeva/syntax-ru/russian.csv') ###КОСТЫЛЬ!!! russian.csv
aug_data = PersonaChatTorchDataset('/home/stc/amatveeva/syntax-ru/russian.csv')###КОСТЫЛЬ!!!
...
with open(log_path, 'w') as log:
    ...
    for fold, (train_ids, val_ids) in enumerate(kfold.split(aug_data)):
        ...
        if has_persona:
            ...
            run(train, val, all_models, lr/6, t_total, 0, has_persona, gradient_accumulation_steps, device, fp16, 
                amp, apply_interaction, matching_method, aggregation_method, epoch_train_losses, epoch_valid_losses, 
                epoch_valid_accs, epoch_valid_recalls, epoch_valid_MRRs, best_model_statedict, writer, save_model_path, test_mode)
